...
const RegisterInput = ({ name, value, check, dupCheck, onChange }: Props) => {
  const lists = [
    {
      ...
      checkUse: dupCheck ? '사용가능한 이메일입니다.' : '중복된 이메일입니다.',
    },
    {
      name: 'nickname',
      title: 'Nickname',
      ...
      warning: '※ 닉네임이 너무 길어요.',
      checkUse: dupCheck ? '사용가능한 닉네임입니다.' : '중복된 닉네임입니다.',
    },
  ];
  ...
  return (
    <>
      <S.LoginInputBox>
        <S.RegisterTitle>{list.title}</S.RegisterTitle>
        <S.MoreInfo>{list.info}</S.MoreInfo>
        <S.LoginInput
          tabIndex={1}
          type="text"
          name={name}
